...
<img src="assets/images/banner.png" alt="Silvia Navarro Banner" width="100%">
...
![Decap CMS](https://img.shields.io/badge/Decap_CMS-FF4088?style=flat-square&logo=decapcms&logoColor=white)
...
Built without heavy UI frameworks, focusing on crisp CSS3 design and a decoupled content management approach.

* **Cinematic Scroll-Snapping:** 100vh sections with magnetic scroll to create an immersive, native-app-like experience.
* **Expanded Headless CMS (Decap CMS):** Integrated via Netlify Identity. The client can access a private `/admin` dashboard to visually edit background videos, the main trailer, polaroid images, titles, and links. 
* **Dynamic UI & Glassmorphism:** A meticulously crafted aesthetic featuring continuous tunnel-lighting gradients, frosted glass effects (`backdrop-filter`), and responsive mathematical scaling (`clamp()`) for the photo collages.
* **Real-time Git Sync:** Every change made in the CMS automatically triggers a `git commit` to this repository, prompting Netlify to rebuild and deploy the site in seconds.
* **Responsive Share Modal:** A custom-built JavaScript modal that perfectly mimics native share sheets, featuring native SVG icons to prevent ad-blocker issues.

## 🔗 Live Demo

Check out the cinematic experience here:
**[View Live Project](https://silvianavarro-links.netlify.app)**
...
Construido sin *frameworks* pesados de UI, apostando por un diseño CSS3 impecable y un enfoque de gestión de contenido desacoplado.
...
* **CMS Headless Expandido (Decap CMS):** Conectado mediante Netlify Identity. El cliente puede acceder a un panel privado `/admin` para editar visualmente vídeos de fondo, el tráiler principal, imágenes tipo polaroid, títulos y enlaces.
* **UI Dinámica y Glassmorphism:** Un diseño meticulosamente elaborado con degradados de iluminación continuos, efectos de cristal esmerilado (`backdrop-filter`) y un escalado matemático *responsive* (`clamp()`) para los collages fotográficos.
* **Sincronización Git en Tiempo Real:** Cada cambio realizado en el panel genera automáticamente un `git commit` en este repositorio, lo que indica a Netlify que debe recompilar y desplegar la web en segundos.
* **Modal de Compartir Responsive:** Un modal creado en JavaScript puro que imita los menús nativos de los móviles, utilizando iconos SVG incrustados para evitar problemas con bloqueadores de anuncios.

## 🔗 Live Demo

Échale un vistazo a la experiencia cinematográfica en vivo aquí:
**[Ver Proyecto en Vivo](https://silvianavarro-links.netlify.app)**
